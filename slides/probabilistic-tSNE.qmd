...
_In a first step, consider a graph with one connected component_
...
Priors for $W_X$ and $W_Z$ induce posteriors $\mathbf{P}^{\mathcal{P}_X}$ and $\mathbf{Q}^{\mathcal{P}_Z}$ which are with cross entropy $\mathcal{H}\big(\mathbf{P}^{\mathcal{P}_X},\mathbf{Q}^{\mathcal{P}_Z}\big)$
