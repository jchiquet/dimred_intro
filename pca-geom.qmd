...
$$
  \minimize_{\Delta} I_{\Delta_1} \Leftrightarrow \maximize_{\Delta^\bot} I_{\Delta_1^\bot}
$$

## Finding the best axis

Let $\bv_1$ be the unitary director vector of $\Delta_1$, normal to $\Delta^\bot$.
...
\begin{equation*}
  \bc_i  = (\bx_i - \bar{\bx})^\top \bV = \bX^c_i \bV, \quad \bc_i \in \Rset^p.
\end{equation*}

In the variable space $\Rset^n$, new variables (factors) are formed by linear combinasions of the orginal variables: the \emphase{principal components} (PC)
